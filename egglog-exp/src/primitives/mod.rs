...
impl Primitives {
    ...
    pub(crate) fn get_schema(&self, id: PrimitiveFunctionId) -> Schema {
        let (args, ret) = self.operations[id].op.signature();
        Schema::new(
            args.iter().copied().map(ColumnTy::Primitive),
            ColumnTy::Primitive(ret),
        )
    }
...
macro_rules! lift_operation_impl {
    ([$arity:expr, $table:expr] fn $name:ident ( $($id:ident : $ty:ty : $n:tt),* ) -> $ret:ty { $body:expr }) => {
         {
            use $crate::primitives::{Primitives, PrimitiveOperation};
