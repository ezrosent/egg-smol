...
fn basic_updates() {
    ...
    let mut index = Index::new(vec![ColumnId::new(0), ColumnId::new(2)], &ps);
...
fn basic_set() {
    let ps = PoolSet::default();
    let mut table = KeySet::new(2, &ps);
    assert!(table.insert(&[v(0), v(1)]));
    assert!(table.insert(&[v(2), v(1)]));
    assert!(table.insert(&[v(2), v(3)]));
    assert!(!table.insert(&[v(0), v(1)]));
}
