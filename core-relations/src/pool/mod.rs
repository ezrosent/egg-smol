...
use crate::{
    ...
    hash_index::{KeyPresenceTable, SubsetTable},
    offsets::{SortedOffsetVector, Subset},
...
impl<T: Clear> Pool<T> {
    ...
    pub(crate) fn get(&self) -> Pooled<T> {
        let empty = if let Some(prev) = self.data.borrow_mut().pop() {
            prev
        } else {
            T::default()
        };
...
impl<T: Clear> Pooled<T> {
    ...
    /// Get an empty value of type `T`, potentially reused from the pool used by `this``.
    pub(crate) fn new(this: &Pooled<T>) -> Pooled<T> {
        this.pool.get()
    }

    /// Remove the wrapped object from the pool and return it.
    pub(crate) fn into_inner(mut this: Pooled<T>) -> T {
        unsafe {
            let item: &T = &this.data;
            // SAFETY: We will not be dropping `this.data` and have ownership of `this`.
            let res: T = ptr::read(item);
            // SAFETY: We are forgetting `this`, `pool` will be dropped the same
            // number of times as if it were normally dropped.
            ptr::drop_in_place(&mut this.pool);
            mem::forget(this);
            res
        }
    }
    ...
    /// type.
    pub(crate) fn transfer_rc(this: Pooled<T>, rc_pool: &Pool<Rc<T>>) -> Pooled<Rc<T>> {
        let item = Pooled::into_inner(this);
        Pooled {
            data: ManuallyDrop::new(Rc::new(item)),
            pool: rc_pool.clone(),
        }
    }
}
...
pool_set! {
    pub PoolSet {
        ...
        pivot_table: HashMap<SmallVec<[Value; 3]>, Subset>,
