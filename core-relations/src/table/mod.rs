...
pub(crate) type MergeFn =
    Box<dyn Fn(&mut ExecutionState, &[Value], &[Value], &mut Vec<Value>) -> bool>;
...
impl SortedWritesTable {
    ...
    fn maybe_rehash(&mut self) {
        let threshold = cmp::max(16, self.data.data.len() * 7 / 8);
        if self.data.stale_rows > threshold {
            self.rehash();
        }
    }
